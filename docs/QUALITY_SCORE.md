...
| Representative fixtures | 3 | Project-owned CSV/binary fixtures plus licensed MoTeC, negative and Traqmate physical-car fixtures are mechanically checked; a redistributable physical-car MoTeC session remains a useful data gap. |
...
The browser source workflow and the first React investigation interface are verified.

The controlled first vertical slice now runs end to end through synchronized telemetry investigation without browser-side engineering recomputation.

Plan 020 validated those assumptions against licensed physical-car telemetry and found a concrete source-ingestion gap without fabricating compatibility.
...
Plan 026 now prepares the selected Portland physical laps onto one canonical common reference and preserves exact projection plateaus without fabricated motion.

The next maturity step is to make base time-vs-distance comparison explicitly plateau-aware while preserving dwell time and rejecting true backtracking.
