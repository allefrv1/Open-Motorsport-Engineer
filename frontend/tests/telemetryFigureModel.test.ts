  ...
  continuous_overlays: [
    {
      ...
      unit: "rad/s",
      distance_grid_m: distance,
      lap_a_values: [500, 510, 520, 530, 540],
      lap_b_values: [505, 515, 525, 535, 545],
    },
    {
    ...
    },
    {
      ...
      unit: "rad",
      distance_grid_m: distance,
      lap_a_values: [0, 0.1, 0.2, 0.1, 0],
      lap_b_values: [0, 0.12, 0.22, 0.08, 0],
    },
  ],
  gear_overlay: {
...
describe("telemetry investigation figure model", () => {
  ...
  it("passes server arrays through without numerical modification", () => {
    ...
    expect(speedPanel.distanceM).toBe(report.continuous_overlays[1].distance_grid_m);
    expect(speedPanel.traces[0].values).toBe(report.continuous_overlays[1].lap_a_values);
    expect(speedPanel.traces[1].values).toBe(report.continuous_overlays[1].lap_b_values);
  });
